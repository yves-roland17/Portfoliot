...
export default function Contact({ profile, isAdmin }: ContactProps) {
  ...
  // Collapsible Developer Keys Configuration Drawer to easily link EmailJS!
          ...
          {showConfig && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <motion.div
                ...
                className="relative bg-white dark:bg-black border border-slate-200/50 dark:border-white/10 rounded-3xl p-6 sm:p-8 w-full max-w-md shadow-2xl z-10 glass backdrop-blur-2xl"
              >
                <button
                  onClick={() => setShowConfig(false)}
                ...
                <div className="space-y-4 text-xs sm:text-sm">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      placeholder="Ex: service_xxxx"
                      className="w-full px-3.5 py-2 rounded-xl border border-slate-250 dark:border-white/5 dark:bg-white/3 text-slate-900 dark:text-white text-xs"
                    />
                  </div>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      placeholder="Ex: xxxxx_xxxxxxxx"
                      className="w-full px-3.5 py-2 rounded-xl border border-slate-250 dark:border-white/5 dark:bg-white/3 text-slate-900 dark:text-white text-xs"
                    />
                  </div>

                  {/* Information block */}
                  <div className="p-3 bg-indigo-50/50 dark:bg-white/3 border border-indigo-100/30 rounded-xl text-indigo-900 dark:text-indigo-400 flex items-start gap-2 text-xs">
                    <Info className="w-4 h-4 shrink-0 mt-0.5" />
                    <p className="leading-relaxed">
                      Saisissez vos identifiants temporaires ici pour envoyer de réels e-mails. Ils sont conservés localement dans la session de votre navigateur, ou définissez les dans votre <strong>.env</strong> pour les persister (<em>VITE_EMAILJS_SERVICE_ID</em>, etc.).
                    </p>
                  </div>

                  <button
                    onClick={() => setShowConfig(false)}
                    className="w-full py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 text-white dark:text-slate-950 text-xs font-semibold cursor-pointer"
                  >
                    Valider et fermer
                  </button>
                </div>
